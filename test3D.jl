...
xmin,ymin,zmin,tmin = 0.0,0.0,0.0,0.0
xmax,ymax,zmax,tmax = xmin+dx*(nx-1),ymin+dy*(ny-1),zmin+dz*(nz-1),tmin+dt*(nt-1)
sx = dx * div(nx,2)
sy = dy * div(ny,2)
sz = 1 * dz
rx = dx*[npad:nx-npad-1;]
ry = div(ny,2) * dy .* ones(length(rx))
...
function make_op(interpmethod, modeltype, fs, v, b; comptype = Float32)
    ...
    elseif modeltype == WaveFD.Prop2DAcoIsoDenQ_DEO2_FDTD_Model_VB
        ...
        m[:,:,:,1] .= reshape(v,nz,ny,nx)
        m[:,:,:,2] .= reshape(b,nz,ny,nx)
    elseif modeltype == WaveFD.Prop2DAcoIsoDenQ_DEO2_FDTD_Model_B
        kwargs = (v = reshape(v, nz, ny, nx), )
...
ky0 = div(ny,2)
ky1 = ky0 - div(boxsize,2)
ky2 = ky0 + div(boxsize,2)
...
b1 =    1 .* ones(Float32, nz, ny, nx, 1);
b2 =    1 .* ones(Float32, nz, ny, nx, 1); 
v1 = 1500 .* ones(Float32, nz, ny, nx, 1);
v2 = 1500 .* ones(Float32, nz, ny, nx, 1); 

v2[kz1:kz2,ky1:ky2,vx1:vx2] .*= 1.25;
b2[kz1:kz2,ky1:ky2,bx1:bx2] .*= 1.50;
...
dmax = 0.05 * maximum(abs,d2); dmin = - dmax 
...
subplot(2,3,1); imshow(reshape(v1[:,div(ny,2),:], shape), cmap="seismic", aspect="auto", vmin=vmin, vmax=vmax); title("3D Velocity Background v1"); colorbar();
subplot(2,3,2); imshow(reshape(v2[:,div(ny,2),:], shape), cmap="seismic", aspect="auto", vmin=vmin, vmax=vmax); title("3D Velocity Perturbed v2"); colorbar();
subplot(2,3,3); imshow(reshape(v3[:,div(ny,2),:], shape), cmap="seismic", aspect="auto", vmin=vmin, vmax=vmax); title("3D Velocity Difference (v2-v1)"); colorbar();
subplot(2,3,4); imshow(reshape(b1[:,div(ny,2),:], shape), cmap="seismic", aspect="auto", vmin=bmin, vmax=bmax); title("3D Buoyancy Background b1"); colorbar();
subplot(2,3,5); imshow(reshape(b2[:,div(ny,2),:], shape), cmap="seismic", aspect="auto", vmin=bmin, vmax=bmax); title("3D Buoyancy Perturbed b2"); colorbar();
subplot(2,3,6); imshow(reshape(b3[:,div(ny,2),:], shape), cmap="seismic", aspect="auto", vmin=bmin, vmax=bmax); title("3D Buoyancy Difference (b2-v1)"); colorbar();
tight_layout()
display(gcf())
savefig("figure.3D.model.png")

scale = 10
@show scale

figure(3,figsize=(18,12))
subplot(2,2,1); imshow(d1, cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Reference F*v1"); colorbar();
subplot(2,2,2); imshow(d2, cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Velocity Only F*v2"); colorbar();
subplot(2,2,3); imshow(d4, cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Buoyancy Only F*b2"); colorbar();
subplot(2,2,4); imshow(d6, cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Velocity + Buoyancy F*vb2"); colorbar();
tight_layout()
display(gcf())
savefig("figure.3D.data.nonlinear.png")

figure(4,figsize=(18,12))
subplot(2,2,2); imshow(scale .* (d2 .- d1), cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Velocity Only F*v2 - F*v1 (($(scale)x)"); colorbar();
...
tight_layout()
display(gcf())
savefig("figure.3D.data.difference.png")

figure(5,figsize=(18,12))
subplot(2,2,2); imshow(scale .* (δd2), cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Velocity Only J*(v2-v1) ($(scale)x)"); colorbar();
subplot(2,2,3); imshow(scale .* (δd3), cmap="seismic", aspect="auto", vmin=dmin, vmax=dmax); title("3D Buoyancy Only J*(b2-b1) (($(scale)x)"); colorbar();
...
tight_layout()
display(gcf())
savefig("figure.3D.data.linear.png")
