function JetProp3DAcoVTIDenQ_DEO2_FDTD(;
    ...
    if "v" ∈ active_modelset_keys && "b" ∉ active_modelset_keys && "ϵ" ∈ active_modelset_keys && "η" ∈ active_modelset_keys
        ...
        modeltype = Wave.Prop3DAcoVTIDenQ_DEO2_FDTD_Model_VEA
    elseif "v" ∈ active_modelset_keys && "b" ∉ active_modelset_keys && "ϵ" ∉ active_modelset_keys && "η" ∉ active_modelset_keys
        ...
        modeltype = Wave.Prop3DAcoVTIDenQ_DEO2_FDTD_Model_V
    else
    ...
    jet = Jet(
        dom = dom,
        rng = rng,
        f! = JopProp3DAcoVTIDenQ_DEO2_FDTD_f!,
        df! = JopProp3DAcoVTIDenQ_DEO2_FDTD_df!,
        df′! = JopProp3DAcoVTIDenQ_DEO2_FDTD_df′!,
        s = (
            modeltype = modeltype,
...
Create a `Jets` nonlinear or linearized operator for 3D pseudo- visco-acoustic, vertical transverse isotropy, 
...
"""
JopNlProp3DAcoVTIDenQ_DEO2_FDTD(;kwargs...) = JopNl(JetProp3DAcoVTIDenQ_DEO2_FDTD(;kwargs...))

@doc (@doc JopNlProp3DAcoVTIDenQ_DEO2_FDTD)
JopLnProp3DAcoVTIDenQ_DEO2_FDTD(; m₀, kwargs...) = JopLn(JetProp3DAcoVTIDenQ_DEO2_FDTD(;kwargs...), m₀)

export JopNlProp3DAcoVTIDenQ_DEO2_FDTD
export JopLnProp3DAcoVTIDenQ_DEO2_FDTD

function JopProp3DAcoVTIDenQ_DEO2_FDTD_nonlinearforward!(d::AbstractArray, m::AbstractArray; kwargs...)
    # make propagator
    nz_ginsu,ny_ginsu,nx_ginsu = size(kwargs[:ginsu])
    z0_ginsu,y0_ginsu,x0_ginsu = origin(kwargs[:ginsu])
    p = Wave.Prop3DAcoVTIDenQ_DEO2_FDTD(
        nz = nz_ginsu,
        ny = ny_ginsu,
        nx = nx_ginsu,
    ...
    for it = 1:ntmod_wav
        ...
        if it >= it0 && rem(it-1,itskip) == 0
            if length(d) > 0
                ...
                Wave.extractdata!(d, wavefields["pold"], div(it-1,itskip)+1, iz_rec, iy_rec, ix_rec, c_rec)
            end
    ...
    JopProp3DAcoVTIDenQ_DEO2_FDTD_stats(kwargs[:stats], kwargs[:ginsu], ntmod_wav, time()-time1, cumtime_io, cumtime_ex)
...
function JopProp3DAcoVTIDenQ_DEO2_FDTD_f!(d::AbstractArray, m::AbstractArray; kwargs...)
    ...
    if !isvalid
        JopProp3DAcoVTIDenQ_DEO2_FDTD_nonlinearforward!(d, m; kwargs...)
...
function JopProp3DAcoVTIDenQ_DEO2_FDTD_df!(δd::AbstractArray, δm::AbstractArray; kwargs...)
    nz_ginsu,ny_ginsu,nx_ginsu = size(kwargs[:ginsu])
    z0_ginsu,y0_ginsu,x0_ginsu = origin(kwargs[:ginsu])
    p = Wave.Prop3DAcoVTIDenQ_DEO2_FDTD(
        nz = nz_ginsu,
    ...
    if !isvalid
        JopProp3DAcoVTIDenQ_DEO2_FDTD_nonlinearforward!(Array{Float32}(undef,0,0), kwargs[:mₒ]; kwargs...)
        kwargs[:srcfieldhost][] = gethostname()
        kwargs[:chksum][] = _chksum
    end
...
function JopProp3DAcoVTIDenQ_DEO2_FDTD_df′!(δm::AbstractArray, δd::AbstractArray; kwargs...)
    nz_ginsu,ny_ginsu,nx_ginsu = size(kwargs[:ginsu])
    z0_ginsu,y0_ginsu,x0_ginsu = origin(kwargs[:ginsu])
    p = Wave.Prop3DAcoVTIDenQ_DEO2_FDTD(
        nz = nz_ginsu,
    ...
    local iz, iy, ix, c
    if kwargs[:interpmethod] == :hicks
        iz, iy, ix, c = Wave.hickscoeffs(kwargs[:dz], kwargs[:dy], kwargs[:dx], z0_ginsu, y0_ginsu, x0_ginsu, 
            nz_ginsu, ny_ginsu, nx_ginsu, kwargs[:rz], kwargs[:ry], kwargs[:rx])
    else
        iz, iy, ix, c = Wave.linearcoeffs(kwargs[:dz], kwargs[:dy], kwargs[:dx], z0_ginsu, y0_ginsu, x0_ginsu, 
            nz_ginsu, ny_ginsu, nx_ginsu, kwargs[:rz], kwargs[:ry], kwargs[:rx])
    end
    for i = 1:length(c)
...
function JopProp3DAcoVTIDenQ_DEO2_FDTD_stats(stats, ginsu, it, cumtime_total, cumtime_io, cumtime_ex, cumtime_im=0.0)
