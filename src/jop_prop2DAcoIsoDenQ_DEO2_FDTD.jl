function JetProp2DAcoIsoDenQ_DEO2_FDTD(;
    ...
    for wavefield_active in ["P","DP"]
        compressor[wavefield_active] = Wave.Compressor(Float32, Float32, C, size(ginsu,interior=isinterior), (nz_subcube,nx_subcube), compscale, ntrec, isinterior)
    end
...
function JopProp2DAcoIsoDenQ_DEO2_FDTD_nonlinearforward!(d::AbstractArray, m::AbstractArray; kwargs...)
    ...
    for i = 1:length(c_sou_scaled)
        for ix = 1:size(c_sou_scaled[i], 2), iz = 1:size(c_sou_scaled[i], 1)
            jz = iz_sou[i][iz]
            jx = ix_sou[i][ix]
            c_sou_scaled[i][iz,ix] *= kwargs[:dtmod]^2 * m_ginsu[jz,jx]^2 / b_ginsu[jz,jx]
        end
    end
    ...
    for it = 1:ntmod_wav
        ...
        if it >= it0 && rem(it-1,itskip) == 0
            if length(d) > 0
                ...
                Wave.extractdata!(d, pold, div(it-1,itskip)+1, iz_rec, ix_rec, c_rec)
            end
...
function JopProp2DAcoIsoDenQ_DEO2_FDTD_df!(δd::AbstractArray, δm::AbstractArray; kwargs...)
    ...
    for it = 1:ntmod
        ...
        if rem(it-1,itskip) == 0
			...
			cumtime_io += @elapsed Wave.compressedread!(iofield, kwargs[:compressor]["DP"], div(it-1,itskip)+1, kwargs[:isinterior] ? interior(kwargs[:ginsu], DP) : DP)
