function JetProp2DAcoTTIDenQ_DEO2_FDTD(;
    ...
    if "v" ∈ active_modelset_keys && "b" ∉ active_modelset_keys && "ϵ" ∈ active_modelset_keys && "η" ∈ active_modelset_keys
        ...
        modeltype = Wave.Prop2DAcoTTIDenQ_DEO2_FDTD_Model_VEA
    elseif "v" ∈ active_modelset_keys && "b" ∉ active_modelset_keys && "ϵ" ∉ active_modelset_keys && "η" ∉ active_modelset_keys
        ...
        modeltype = Wave.Prop2DAcoTTIDenQ_DEO2_FDTD_Model_V
    else
...
function JopProp2DAcoTTIDenQ_DEO2_FDTD_nonlinearforward!(d::AbstractArray, m::AbstractArray; kwargs...)
    ...
    model_ginsu = Dict("v"=>Wave.V(p), "b"=>Wave.B(p), "ϵ"=>Wave.Eps(p), "η"=>Wave.Eta(p), "f"=>Wave.F(p), 
        "cosθ"=>Wave.CosTheta(p), "sinθ"=>Wave.SinTheta(p))
...
function JopProp2DAcoTTIDenQ_DEO2_FDTD_f!(d::AbstractArray, m::AbstractArray; kwargs...)
    d .= 0
...
function srcillum!(γ, A::T, m::AbstractArray) where {D,R,J<:Jet{D,R,typeof(JopProp2DAcoTTIDenQ_DEO2_FDTD_f!)},T<:Jop{J}}
    s = state(A)
